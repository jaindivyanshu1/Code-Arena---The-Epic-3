...
const runcode = (dir, input, req, res) => {
  ...
  const command = input
    ? ` cd ${dir} &&  javac ${filepath}.java && java ${filepath} < ${inputpath}`
    : ` cd ${dir} && javac ${filepath}.java && java ${filepath}`;
...
// const runcode = (dir, input, req, res) => {
//   const filepath = "Solution.java";
//   const inputpath = "input.txt";
//   const command = input
//     ? ` cd ${dir} &&  javac ${filepath}.java && java ${filepath} < ${inputpath}`
//     : ` cd ${dir} && javac ${filepath} && java ${filepath}`;
//   exec(command, { timeout: 5000 }, (err, stdout, stderr) => {
//     if (err) {
//       console.log(err);
//       res.json({ val: "err" });
  ...
  exec(command, (err, stdout, stderr) => {
    if (err) {
      ...
      if (stdout.includes("0")) {
        res.json({
          message: `wrong answer in ${stdout.indexOf("0")} testcase`,
          status: "0",
        });
      } else {
...
router.post("/judge", async (req, res) => {
  try {
    ...
    const sql = `select java_code,input_file,java_code_runner from problems where problemID = "3"`;
